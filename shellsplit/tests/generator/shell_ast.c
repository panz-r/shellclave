...
ast_node_t *shell_ast_add_pipeline(shell_ast_t *ast, ast_node_t *cmd1,
                                   ast_node_t *cmd2) {
  if (!ast || !cmd1 || !cmd2)
    return NULL;
...
ast_node_t *shell_ast_add_sequence(shell_ast_t *ast, ast_node_t *cmd1,
                                   ast_node_t *cmd2, const char *separator) {
  if (!ast || !cmd1 || !cmd2)
    return NULL;
...
ast_node_t *shell_ast_add_redirect(shell_ast_t *ast, ast_node_t *cmd,
                                   const char *target, int fd, bool is_input,
                                   bool is_append, bool is_stderr) {
  if (!ast || !cmd)
    return NULL;
...
ast_node_t *shell_ast_add_variable(shell_ast_t *ast, const char *name,
                                   bool is_braced) {
  if (!ast || !name)
    return NULL;
...
ast_node_t *shell_ast_add_arithmetic(shell_ast_t *ast, const char *expr,
                                     bool is_unclosed) {
  if (!ast || !expr)
    return NULL;
...
ast_node_t *shell_ast_add_heredoc(shell_ast_t *ast, const char *delimiter,
                                  const char *content) {
  if (!ast || !delimiter)
    return NULL;
...
ast_node_t *shell_ast_add_process_sub(shell_ast_t *ast, ast_node_t *cmd,
                                      bool is_input) {
  if (!ast || !cmd)
    return NULL;
...
static size_t serialize_node(const ast_node_t *node, char *buffer,
                             size_t buffer_size, size_t pos);

static size_t append_str(char *buffer, size_t buffer_size, size_t pos,
                         const char *str) {
  if (!buffer || !str)
    return pos;
...
static size_t serialize_node(const ast_node_t *node, char *buffer,
                             size_t buffer_size, size_t pos) {
  if (!node || !buffer || pos >= buffer_size)
    return pos;
  ...
  case AST_COMMAND:
    ...
    if (node->has_redirect && node->child &&
        node->child->type == AST_REDIRECT) {
...
char *shell_ast_serialize(const shell_ast_t *ast, char *buffer,
                          size_t buffer_size) {
  if (!ast || !buffer || buffer_size == 0)
    return NULL;
...
ast_node_t *shell_ast_add_if(shell_ast_t *ast, ast_node_t *condition,
...
ast_node_t *shell_ast_add_loop(shell_ast_t *ast, const char *type,
                               const char *var, ast_node_t *list,
                               ast_node_t *body) {
  if (!ast)
    return NULL;
...
ast_node_t *shell_ast_add_case(shell_ast_t *ast, const char *var,
...
ast_node_t *shell_ast_add_quote(shell_ast_t *ast, const char *content,
                                char quote_char, bool is_closed) {
  if (!ast)
    return NULL;
...
ast_node_t *shell_ast_add_backtick(shell_ast_t *ast, const char *content,
                                   bool is_closed) {
  if (!ast)
    return NULL;
  ast_node_t *node = ast_node_create(AST_BACKTICK);
  if (node) {
    node->value = strdup(content ? content : "");
    node->is_valid = is_closed;
    ast->node_count++;
