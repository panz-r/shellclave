...
int shell_interop_parse(shell_interop_handle_t *handle, const char *cmd,
   ...
   * the input. */
  if (memchr(cmd, '\0', (size_t)cmd_len) != NULL) {
    return 0;
  }
...
char *shell_interop_subcommand_str(shell_interop_handle_t *handle, int i) {
  ...
  shell_copy_subcommand(handle->cmd_buffer, &handle->result.cmds[i], buf,
                        handle->result.cmds[i].len + 1);
  return buf;
}
...
char *shell_interop_features_str(int features) {
  ...
  if (features & SHELL_FEAT_VARS)
    strcat(buf, "VAR ");
  if (features & SHELL_FEAT_GLOBS)
    strcat(buf, "GLOB ");
  if (features & SHELL_FEAT_SUBSHELL)
    strcat(buf, "SUBSHELL ");
  if (features & SHELL_FEAT_ARITH)
    strcat(buf, "ARITH ");
  if (features & SHELL_FEAT_HEREDOC)
    strcat(buf, "HEREDOC ");
  if (features & SHELL_FEAT_HERESTRING)
    strcat(buf, "HERESTRING ");
  if (features & SHELL_FEAT_PROCESS_SUB)
    strcat(buf, "PROCSUB ");
  if (features & SHELL_FEAT_LOOPS)
    strcat(buf, "LOOPS ");
  if (features & SHELL_FEAT_CONDITIONALS)
    strcat(buf, "COND ");
  if (features & SHELL_FEAT_CASE)
    strcat(buf, "CASE ");
