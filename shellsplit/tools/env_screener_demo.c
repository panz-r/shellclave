...
void print_value_analysis(const char *name, const char *value) {
  ...
  printf("%sIs empty: %s\n", INDENT,
         is_empty_value(value) ? "YES (skip)" : "NO");
}
...
void print_whitelist_check(const char *name) {
  ...
  if (!is_whitelisted) {
    printf("%sKnown safe vars (partial): DISPLAY, PATH, HOME, SSH_AUTH_SOCK, "
           "etc.\n",
           INDENT);
  }
}
...
void print_entropy_analysis(const char *value, bool has_prefix,
  ...
  if (rel_conditional > 1.0) {
    printf("%s    >1.0 means original is MORE random than permutations "
           "(suspicious)\n",
           INDENT);
  } else {
    printf(
        "%s    <1.0 means original has structure that permutations destroy\n",
        INDENT);
  }
...
void print_bayesian_scoring(double shannon, double rel_conditional,
                            double rel_2gram, bool has_prefix,
  ...
  double lh_ns_cond =
      (fabs(deviation) < 0.3)
          ? 0.1
          : (0.5 + 0.45 * exp(-0.5 * pow((fabs(deviation) - 0.3) / 0.2, 2)));
  printf(
      "%s    Conditional ratio: %.4f -> P(E|secret)=%.4f, P(E|~secret)=%.4f\n",
  ...
  printf(
      "%s    Shannon normalized: %.4f -> P(E|secret)=%.4f, P(E|~secret)=%.4f\n",
      INDENT, shannon_norm, lh_shannon, lh_ns_shannon);

  posterior = posterior * lh_s_cond /
              (posterior * lh_s_cond + (1.0 - posterior) * lh_ns_cond);
  printf("%s    Posterior after [A]: %.4f\n", INDENT, posterior);

  posterior = posterior * lh_s_2gram /
              (posterior * lh_s_2gram + (1.0 - posterior) * lh_ns_cond);
  printf("%s    Posterior after [B]: %.4f\n", INDENT, posterior);

  posterior = posterior * lh_shannon /
              (posterior * lh_shannon + (1.0 - posterior) * lh_ns_shannon);
  printf("%s    Posterior after [C]: %.4f\n", INDENT, posterior);

  printf("\n%s  [B] Prefix evidence: %s\n", INDENT,
         has_prefix ? "PRESENT" : "ABSENT");
  double lh_p_secret = has_prefix ? 0.98 : 0.12;
  double lh_p_not = has_prefix ? 0.02 : 0.88;
  posterior = posterior * lh_p_secret /
              (posterior * lh_p_secret + (1.0 - posterior) * lh_p_not);
  printf("%s    Posterior after [D]: %.4f\n", INDENT, posterior);

  printf("\n%s  [C] Name pattern evidence: %s\n", INDENT,
         has_name_pattern ? "PRESENT" : "ABSENT");
  double lh_n_secret = has_name_pattern ? 0.90 : 0.15;
  double lh_n_not = has_name_pattern ? 0.10 : 0.85;
  posterior = posterior * lh_n_secret /
              (posterior * lh_n_secret + (1.0 - posterior) * lh_n_not);
  printf("%s    Posterior after [E]: %.4f\n", INDENT, posterior);

  printf("\n%s  [D] Path evidence: %s\n", INDENT,
         is_path ? "PRESENT (negative)" : "ABSENT");
  double lh_path_secret = is_path ? 0.01 : 0.95;
  double lh_path_not = is_path ? 0.99 : 0.05;
  posterior = posterior * lh_path_secret /
              (posterior * lh_path_secret + (1.0 - posterior) * lh_path_not);
...
void print_threshold_decision(double score, double threshold) {
  ...
  printf("%sScore: %.4f\n", INDENT, score);
  printf("%sDecision: %s\n", INDENT,
         score > threshold ? "FLAGGED (would block/prompt)" : "CLEAR");
}

void print_summary(const char *name, const char *value, double score,
                   bool is_whitelisted, bool passes_min_length,
                   double threshold) {
  print_separator();
  printf("SUMMARY: %s=%s (len=%zu)\n", name, safe_value_str(value),
         strlen(value));
  print_separator();
...
void print_usage(const char *prog) {
  ...
  fprintf(stderr,
          "The VALUE is redacted in output to avoid exposing secrets.\n");
  fprintf(stderr, "\nOptions:\n");
...
int main(int argc, char **argv) {
  ...
  const char *eq = strchr(var_eq_value, '=');
  if (!eq) {
